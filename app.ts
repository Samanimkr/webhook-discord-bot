...
let ukDateFormatter: Intl.DateTimeFormat | null = null;
let ukDateFormatterFailed = false;
...
async function handleWebhook(payload: WebhookPayload, env: Env) {
    ...
    if (payload.type === "server_failed" && env.RENDER_API_KEY) {
        try {
    ...
    try {
        await sendEventMessage(payload, event, env);
    } catch (error) {
        console.error(error);
    }
}
...
async function sendEventMessage(
    ...
    const title = formatEventTitle(payload, presentation);
    const color = presentation.color;
    const fields = buildEventFields(payload, event);
...
function formatEventDescription(
    ...
    if (payload.type === "server_failed") {
        return event?.details?.reason ? String(event.details.reason) : "";
    }
    return "";
}
...
function formatTimestamp(timestamp: string): string {
    ...
    const formatter = getUkDateFormatter();
    if (!formatter) {
...
function getUkDateFormatter(): Intl.DateTimeFormat | null {
    if (ukDateFormatterFailed) {
        return null;
    }
    if (ukDateFormatter) {
        return ukDateFormatter;
    }
    try {
        ukDateFormatter = new Intl.DateTimeFormat("en-GB", {
            year: "numeric",
            month: "short",
            day: "2-digit",
            hour: "2-digit",
